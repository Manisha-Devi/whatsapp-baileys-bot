...
function getBookingEntriesForBus(busCode) {
  const prefix = `${busCode}_`;

  return Object.entries(bookingsDb.data || {})
    .map(([id, record]) => {
      if (!id.startsWith(prefix) || !record || typeof record !== "object") {
        return null;
      }
      ...
      return { id, record, date };
    })
    .filter(Boolean)
    .sort((a, b) => b.date - a.date);
}

function parseBookingMonthQuery(text, now = new Date()) {
  const monthNames = [
  ...
  if (text === "this month") {
    return { year: now.getFullYear(), month: now.getMonth(), label: "This Month" };
  }

  if (text === "last month") {
    const lastMonth = new Date(now.getFullYear(), now.getMonth() - 1, 1);
    return { year: lastMonth.getFullYear(), month: lastMonth.getMonth(), label: "Last Month" };
  }

  const monthMatch = text.match(/^([a-z]{3,9})(?:\s+(\d{4}))?$/i);
  if (!monthMatch) return null;

  const monthIndex = monthNames.findIndex((name) => name.startsWith(monthMatch[1]));
...
function getBookingAmount(field) {
  if (field === undefined || field === null || field === "") return 0;
  if (typeof field === "object") return Number(field.Amount ?? field.amount) || 0;
  return Number(field) || 0;
}

async function sendBookingRecord(sock, sender, record, title) {
  const dateStart = record.Date?.Start || "___";
  const dateEnd = record.Date?.End || dateStart;
  const dateDisplay = dateStart === dateEnd ? dateStart : `${dateStart} to ${dateEnd}`;
  const extraExpenses = (record.ExtraExpenses || [])
    .map((expense) => `🧾 ${expense.name || "Expense"}: ₹${getBookingAmount(expense.amount).toLocaleString("en-IN")}${expense.mode === "online" ? " (Online)" : ""}`)
    .join("\n");
  const employeeExpenses = (record.EmployExpenses || [])
    .map((expense) => `👤 ${expense.role || expense.name || "Employee"}: ₹${getBookingAmount(expense.amount).toLocaleString("en-IN")}${expense.mode === "online" ? " (Online)" : ""}`)
    .join("\n");
  const paymentHistory = (record.PaymentHistory || [])
    .map((payment) => `💰 ${payment.date || "Received"}: ₹${getBookingAmount(payment.amount).toLocaleString("en-IN")}${payment.mode === "online" ? " (Online)" : ""}`)
    .join("\n");

  const postBookingDetails = [
    record.Diesel ? `⛽ Diesel: ₹${getBookingAmount(record.Diesel).toLocaleString("en-IN")}` : "",
    record.Adda ? `🚌 Adda: ₹${getBookingAmount(record.Adda).toLocaleString("en-IN")}` : "",
    record.Union ? `🤝 Union: ₹${getBookingAmount(record.Union).toLocaleString("en-IN")}` : "",
    record.CashHandover ? `💵 Cash Hand Over: ₹${getBookingAmount(record.CashHandover).toLocaleString("en-IN")}` : "",
  ].filter(Boolean).join("\n");

  const message = [
    title,
    `🚌 Bus: *${record.BusCode || "___"}*`,
    "",
    "👤 *Customer Details:*",
    `👤 Name: ${record.CustomerName || "___"}`,
    `📱 Mobile: ${record.CustomerPhone || "___"}`,
    "",
    "📍 *Route Details:*",
    `🚏 Pickup: ${record.Location?.Pickup || record.PickupLocation || "___"}`,
    `🏁 Drop: ${record.Location?.Drop || record.DropLocation || "___"}`,
    `📅 Travel Date: ${dateDisplay}`,
    `🗓️ Days: ${record.Date?.NoOfDays || 1}`,
    "",
    "💰 *Payment Details:*",
    `💵 Total Fare: ₹${getBookingAmount(record.TotalFare).toLocaleString("en-IN")}`,
    `💳 Advance Paid: ₹${getBookingAmount(record.AdvancePaid).toLocaleString("en-IN")}`,
    `💸 Balance: ₹${getBookingAmount(record.BalanceAmount).toLocaleString("en-IN")}`,
    `📊 Status: ${record.Status || "Pending"}`,
    extraExpenses ? `\n🧾 *Extra Expenses:*\n${extraExpenses}` : "",
    employeeExpenses ? `\n👥 *Employee Expenses:*\n${employeeExpenses}` : "",
    paymentHistory ? `\n💳 *Payment History:*\n${paymentHistory}` : "",
    postBookingDetails ? `\n🔧 *Trip/Collection Details:*\n${postBookingDetails}` : "",
    record.Remarks ? `\n📝 *Remarks:* ${record.Remarks}` : "",
    "",
    "✅ Booking fetched successfully!",
  ].filter(Boolean).join("\n");

  await safeSendMessage(sock, sender, { text: message });
}

/**
 * Sends booking entries one by one for latest-entry and month queries.
 * Supported formats:
 * - "entries" - latest 5 bookings
 * - "entries N" / "last N entries" - latest N bookings
 * - "this month" / "last month" / "July" / "July 2025"
 */
export async function handleBookingEntriesCommand(sock, sender, normalizedText, user) {
  const text = normalizedText.toLowerCase().trim();
  const countMatch = text.match(/^(?:entries?\s+(\d+)|last\s+(\d+)\s+entries?)$/i);
  const isDefaultEntries = text === "entry" || text === "entries";
  const monthQuery = parseBookingMonthQuery(text);

  if (!isDefaultEntries && !countMatch && !monthQuery) return false;
  ...
  const entriesCount = isDefaultEntries
    ? 5
    : countMatch
      ? parseInt(countMatch[1] || countMatch[2], 10)
      : null;

  if (entriesCount !== null && (!Number.isInteger(entriesCount) || entriesCount < 1)) {
    await safeSendMessage(sock, sender, {
      text: "⚠️ Please enter a valid number, for example: *Entries 5*.",
    });
    return true;
  }
  ...
  try {
    ...
    const allEntries = getBookingEntriesForBus(busCode);
    const entries = monthQuery
      ? allEntries.filter(
          ({ date }) =>
            date.getFullYear() === monthQuery.year &&
            date.getMonth() === monthQuery.month
        )
      : allEntries.slice(0, entriesCount);

    if (entries.length === 0) {
      const periodLabel = monthQuery ? ` in *${monthQuery.label}*` : "";
      await safeSendMessage(sock, sender, {
        text: `⚠️ No bookings found for *${busCode}*${periodLabel}.`,
      });
      return true;
    }
    ...
    for (let index = 0; index < entries.length; index++) {
      ...
      const title = monthQuery
        ? `✅ ${monthQuery.label} - Booking ${index + 1} of ${entries.length}`
        : `✅ Booking ${index + 1} of ${entries.length}`;
      await sendBookingRecord(sock, sender, record, title);
    ...
    console.error("❌ Error handling booking entries command for", sender, ":", err);
    await safeSendMessage(sock, sender, {
      text: "❌ Failed to fetch booking entries. Please try again.",
    });
    return true;
  }
}
